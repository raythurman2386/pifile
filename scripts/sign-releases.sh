...
#
# For each VERSION (a tag like v0.1.0), downloads the release's checksums.txt
# from GitHub, signs it with the Ed25519 secret key, and writes the result to
# ~/.pifile/signing/releases/<version>/checksums.txt.sig. The checksums.txt is
...
SIGNING_DIR="${PIFILE_SIGNING_DIR:-$HOME/.pifile/signing}"
SECRET_KEY="${PIFILE_SIGNING_KEY:-$SIGNING_DIR/secret.pem}"
...
Sign the checksums.txt of each published release VERSION (e.g. v0.1.0).
...
Environment:
  PIFILE_SIGNING_DIR   Override the signing directory (default: ~/.pifile/signing)
  PIFILE_SIGNING_KEY   Override the secret key path
EOF
}
...
if [[ ! -f "$SECRET_KEY" ]]; then
    ...
    echo "Generate one with scripts/gen-signing-key.sh first." >&2
    exit 1
fi
