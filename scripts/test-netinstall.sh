...
# mode (PIFILE_RELEASE_BASE_URL pointing at a filesystem path). Verifies that:
...
TARBALL="pifile-${VERSION}-${TRIPLE}.tar.gz"
...
make_fake_tarball() {
    local stage="$WORK/stage/pifile-${VERSION}-${TRIPLE}"
    rm -rf "$WORK/stage"
    mkdir -p "$stage"
...
mkdir -p "$PREFIX/bin"
printf '#!/usr/bin/env bash\necho "pifile %s (fake)"\n' "$(basename "$0")" > "$PREFIX/bin/pifile"
chmod +x "$PREFIX/bin/pifile"
EOF
    chmod +x "$stage/install.sh"
    tar -czf "$RELEASE_DIR/$TARBALL" -C "$WORK/stage" "pifile-${VERSION}-${TRIPLE}"
}
...
run_install() {
    ...
    PIFILE_RELEASE_BASE_URL="$WORK/release" \
...
[[ -x "$INSTALL_DIR/bin/pifile" ]] || fail "pifile binary not installed"
...
[[ ! -e "$INSTALL_DIR/bin/pifile" ]] || fail "tampered tarball must not be installed"
...
[[ ! -e "$INSTALL_DIR/bin/pifile" ]] || fail "tarball must not be installed on bad signature"
...
[[ ! -e "$INSTALL_DIR/bin/pifile" ]] || fail "tarball must not be installed without a signature"
