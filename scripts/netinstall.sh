...
# Install pifile from a prebuilt GitHub Release tarball.
#
...
main() {
    ...
    local tarball="pifile-${version_no_v}-${triple}.tar.gz"

    echo "==> Platform: $triple"
    echo "==> Version:  $version_tag"
    echo "==> Prefix:   $PREFIX"

    if [[ -x "$PREFIX/bin/pifile" ]] && [[ "$FORCE" != true ]]; then
        echo "==> pifile is already installed at $PREFIX/bin/pifile"
    ...
    local package_dir
    package_dir="$(find "$extract_dir" -maxdepth 1 -type d -name 'pifile-'"$version_no_v"'-*' | head -1)"
    ...
    echo "==> Installed pifile $version_tag into $PREFIX"
}
