...
# Generate an Ed25519 signing keypair for pifile release signing.
#
...
# in the installer (scripts/netinstall.sh).
#
...
OUT_DIR="${1:-$HOME/.pifile/signing}"

usage() {
    cat <<EOF
Usage: $0 [OUT_DIR]

Generate an Ed25519 keypair for signing pifile releases.

  OUT_DIR  Directory to write the keys (default: \$HOME/.pifile/signing)

Writes:
  OUT_DIR/secret.pem   Secret key — KEEP OFFLINE, never commit
  OUT_DIR/public.pem   Public key — commit as pifile-signing-key.pub

After generating, commit the contents of public.pem as
pifile-signing-key.pub at the repo root and pin it in the installers.
EOF
}
...
echo "==> Public key: $OUT_DIR/public.pem  (commit as pifile-signing-key.pub)"
echo ""
